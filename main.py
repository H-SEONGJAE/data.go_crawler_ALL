...
# ==========================================
# 2-2. 파일데이터 다운로드 작업 상태 저장/복구 보조 함수
# ==========================================

DOWNLOAD_TASK_SESSION_KEY = "file_download_task3"


def _download_jobs_dir() -> Path:
...
def _latest_download_task_path() -> Path:
    return _download_jobs_dir() / "latest_download_task.json"
...
def _load_file_download_task():
    task = st.session_state.get(DOWNLOAD_TASK_SESSION_KEY)
    if isinstance(task, dict) and task.get("result_path") and task.get("progress_path"):
        return task

    task = _read_json_safely(_latest_download_task_path())
    if isinstance(task, dict) and task.get("result_path") and task.get("progress_path"):
        st.session_state[DOWNLOAD_TASK_SESSION_KEY] = task
        return task

    return None


def _save_file_download_task(task):
    st.session_state[DOWNLOAD_TASK_SESSION_KEY] = task
    _write_json_safely(_latest_download_task_path(), task)


def start_file_download_task(
    inst_name: str,
    org_url: str,
    include_past: bool,
    output_root: str,
    ...
    progress_path = job_dir / f"download_progress_{job_id}.json"
    log_path = job_dir / f"download_log_{job_id}.txt"
    ...
    config = {
        ...
        "expected_pages": max(int(expected_pages or 1), 1),
        "total_count": int(total_count or 0),
        "progress_path": str(progress_path),
    }

    _write_json_safely(config_path, config)

    initial_progress = {
        "state": "running",
        "percent": 1,
        "message": "파일데이터 다운로드 작업을 시작하는 중입니다.",
        "raw_message": "",
        "current": 0,
    ...
    _write_json_safely(progress_path, initial_progress)
    ...
    _save_file_download_task(task)
    return task


def render_file_download_task_panel(task):
    """저장된 진행상황 파일을 읽어 기존과 같은 진행바/완료 ZIP 버튼을 표시합니다."""
    if not task:
        return

    progress_path = Path(task.get("progress_path", ""))
    ...
    st.caption("다운로드 진행상황은 화면이 새로고침되더라도 가능한 범위에서 자동 복구됩니다.")

    result = None
    while True:
        progress = _read_json_safely(progress_path) or {}
        percent = clamp_percent(progress.get("percent", 1))
        message = clean_text(progress.get("message") or "파일데이터 다운로드 진행상황을 확인 중입니다.")
        update_progress(percent, message)

        result = _read_json_safely(result_path)
    ...
    progress = _read_json_safely(progress_path) or {}
    if result.get("ok"):
        zip_path = result.get("zip_path", "") or progress.get("zip_path", "")
        ...
        clear_progress_status()
        st.error(f"🚨 파일 다운로드 중 오류가 발생했습니다: {result.get('error', '')}")
...
# ==========================================
    ...
    if info3 and info3.get("total_pages", 0) > 0:
        ...
        if run_download:
            ...
            with st.spinner(f"'{org}' 파일데이터 다운로드 작업을 시작하는 중입니다..."):
                try:
                    start_file_download_task(
                        inst_name=org,
                        org_url=org_url,
                        ...
                        total_count=int(info3.get("total_count") or 0),
                    )
                except Exception as e:
                    st.error(f"🚨 파일 다운로드 작업 시작 중 오류가 발생했습니다: {e}")

    download_task3 = _load_file_download_task()
    if download_task3:
        st.markdown("---")
        render_file_download_task_panel(download_task3)
