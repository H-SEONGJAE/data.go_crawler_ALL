...
def collect_detail_urls_by_org(org_name: str, total_pages: int = 0, progress_callback=None) -> List[str]:
    """
    기관별 FILE 목록에서 상세 URL만 빠르게 수집합니다.
    currentPage가 끝 페이지 이후 같은 목록을 반복 반환하는 경우를 시그니처로 감지해 종료합니다.
    """
    detail_urls = []
    seen_urls = set()
    seen_signatures = set()
    ...
    per_page = 100
    # total_pages는 10건 기준 페이지 수이므로 100건 기준으로 변환
    max_fast_pages = max(1, math.ceil((int(total_pages or 1) * 10) / per_page))
    # 총건수 파싱 실패 가능성을 대비해 여유분 부여
    max_fast_pages = max(max_fast_pages, 1)

    for page_no in range(1, max_fast_pages + 5):
