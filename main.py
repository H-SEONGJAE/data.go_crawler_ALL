...
def run_file_download_subprocess(
    ...
    cmd = [sys.executable, str(runner_path), str(config_path), str(result_path)]
    ...
    if info3 and info3.get("total_pages", 0) > 0:
        ...
        if run_download:
            ...
            def update_status(msg):
                msg = clean_text(msg)
                percent = 5
                page_match = re.search(r"파일데이터 목록\s+(\d+)페이지", msg) or re.search(r"(\d+)페이지 완료", msg)
                dataset_match = re.search(r"\[(\d+)\s*/\s*(\d+)\]", msg)

                if page_match:
                    page_no = int(page_match.group(1))
                    percent = 5 + min(25, int((page_no / expected_pages) * 25))
                elif "다운로드 대상 데이터셋" in msg:
                    percent = 30
                elif dataset_match:
