...
if menu.startswith("1."):
    ...
    st.markdown(
        ...
        "기본은 브라우저를 쓰지 않는 requests 기반 검색입니다. 입력한 키워드로 파일데이터를 검색하고, 검색 결과 상세페이지의 <b>제공기관</b> 값을 읽어 후보 기관을 만듭니다. "
        "기관을 선택하면 상세페이지 제공기관 값으로 한 번 더 교차 검증해 최종 수집 URL 목록을 확정합니다."
        "</div></div>",
        unsafe_allow_html=True,
    )

    col_a, col_b, col_c = st.columns([4, 1.2, 1.6])
    with col_a:
    ...
    with col_c:
        use_browser_search = st.checkbox("브라우저 기반 검색", value=False, help="기본은 requests 기반 검색입니다. Playwright 설치 오류가 날 경우 이 옵션을 끄고 사용하세요.")
