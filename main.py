...
def make_progress_box(initial_text: str = "진행 준비 중..."):
    """기존 문구/수치는 유지하면서 막대바 진행률만 공통으로 표시합니다."""
    progress_bar = st.progress(0, text=initial_text)
    status_box = st.empty()

    def update(percent=None, message: str = ""):
        pct = 0 if percent is None else clamp_percent(percent)
        progress_bar.progress(pct, text=message or initial_text)
        if message:
    ...
    return progress_bar, status_box, update, clear_status


def make_metadata_progress_callback(update_func):
    """crawler_metadata.py의 진행 payload를 Streamlit progress bar로 변환합니다."""
    def callback(payload):
        if isinstance(payload, str):
            update_func(None, payload)
            return
        ...
        if percent is None:
            if stage in ["list", "list_start"]:
                ...
                if max_pages > 0:
                    percent = 5 + min(25, int((page / max_pages) * 25))
                else:
                    percent = 10
        ...
        update_func(percent, message)

    return callback


def run_file_download_subprocess(
    inst_name: str,
    org_url: str,
    include_past: bool,
    output_root: str,
    ...
    """파일데이터 다운로드만 별도 Python 프로세스에서 실행하고 ZIP 경로를 반환합니다."""
    app_dir = Path(__file__).resolve().parent
    runner_path = app_dir / "download_runner.py"
    if not runner_path.exists():
        raise RuntimeError("download_runner.py 파일을 main.py와 같은 위치에 배치해야 합니다.")

    job_dir = app_dir / "_download_jobs"
    job_dir.mkdir(parents=True, exist_ok=True)

    job_id = f"{int(time.time())}_{random.randint(1000, 9999)}"
    config_path = job_dir / f"download_config_{job_id}.json"
    result_path = job_dir / f"download_result_{job_id}.json"
    ...
    cmd = [sys.executable, "-u", str(runner_path), str(config_path), str(result_path)]
    env = os.environ.copy()
    ...
    if not result_path.exists():
        tail = "\n".join(last_lines[-20:])
        raise RuntimeError(f"다운로드 runner 결과 파일이 생성되지 않았습니다. return_code={return_code}\n{tail}")

    with open(result_path, "r", encoding="utf-8") as f:
        result = json.load(f)

    if return_code != 0 or not result.get("ok"):
        error_text = result.get("error") or "파일데이터 다운로드 subprocess 실행 중 오류가 발생했습니다."
        tail = "\n".join(last_lines[-20:])
        raise RuntimeError(f"{error_text}\n{tail}")

    zip_path = result.get("zip_path", "")
    if not zip_path:
        raise RuntimeError("다운로드 runner가 ZIP 경로를 반환하지 않았습니다.")
...
with st.sidebar:
    st.markdown(
        ...
        .hamburger-icon { font-size: 1.5rem; color: #4B5563; line-height: 1; }
        .menu-text { font-size: 1.25rem; font-weight: 600; color: #4B5563; }
        </style>
    ...
    menu = option_menu(
        ...
        styles={
            ...
            "icon": {"color": "#4B5563", "font-size": "18px"},
            "nav-link": {"font-size": "14.5px", "text-align": "left", "margin": "6px 0px", "--hover-color": "#F3F4F6", "border-radius": "8px", "color": "#4B5563"},
            "nav-link-selected": {"background-color": "#EF4444", "color": "#4B5563", "font-weight": "bold", "border-radius": "8px"},
        },
    )
...
if menu == "메타데이터 Crawler":
    ...
    with tab1:
        ...
        if st.button("전체 수집 실행", type="primary", use_container_width=True, key="run_all_metadata"):
            progress_bar, progress_status_box, update_progress, clear_progress_status = make_progress_box("전체 메타데이터 수집 준비 중...")
            metadata_progress_callback = make_metadata_progress_callback(update_progress)
            log_box = st.empty()
            with st.spinner("전체 메타데이터 수집 중입니다..."):
                log_buffer = io.StringIO()
                try:
                    update_progress(1, "Playwright Chromium 설치 상태 확인 중...")
                    with contextlib.redirect_stdout(log_buffer):
                        crawler_data.ensure_playwright_browser_installed(status_callback=lambda msg: update_progress(2, msg))
                        crawler_metadata.main(progress_callback=metadata_progress_callback)
                    update_progress(100, "전체 메타데이터 수집이 완료되었습니다.")
                    ...
                    update_progress(100, "전체 수집 중 오류가 발생했습니다. 실행 로그를 확인하세요.")
                    log_box.text_area("실행 로그", log_buffer.getvalue()[-10000:], height=300)
...
# ==========================================
    ...
    prep_col_btn2, prep_col_msg2 = st.columns([1.4, 3.6])

    with prep_col_btn2:
        prepare_clicked2 = st.button(
            "필수 라이브러리 설치/점검",
            icon=":material/build:",
            use_container_width=True,
            key="prepare_required_runtime_btn2",
        )

    with prep_col_msg2:
        if st.session_state.get("required_runtime_ready2"):
    ...
    if prepare_clicked2:
        with st.spinner("Playwright Chromium 설치 및 실행 가능 여부를 확인 중입니다..."):
            ok2, msg2 = page1_org_metadata._prepare_required_runtime_before_org_search()

        st.session_state.required_runtime_ready2 = bool(ok2)
    ...
    if not st.session_state.get("required_runtime_ready2"):
    ...
    if info and info.get("total_pages", 0) > 0:
        ...
        if run_clicked:
            org = info["org_name"]
            expected_pages = max(1, math.ceil(int(info.get("total_count") or 0) / 100))
            progress_bar, status_box, update_progress, clear_progress_status = make_progress_box("조회수/다운로드수 수집 준비 중...")

            def update_status(msg):
                msg = clean_text(msg)
                percent = 5
                page_match = re.search(r"목록\s+(\d+)페이지", msg) or re.search(r"(\d+)페이지 완료", msg)
                if page_match:
                    page_no = int(page_match.group(1))
                    percent = 5 + min(90, int((page_no / expected_pages) * 90))
                if "수집 완료" in msg:
                    percent = 100
                update_progress(percent, msg)
            ...
            with st.spinner(f"'{org}' 조회수/다운로드수 수집 중..."):
                try:
                    ...
                    update_progress(100, f"조회수/다운로드수 수집 완료: 총 {len(df)}건")
                    clear_progress_status()
                    if df.empty:
                    ...
                    update_progress(100, "조회수/다운로드수 수집 중 오류가 발생했습니다.")
                    clear_progress_status()
...
# ==========================================
    ...
    st.markdown("**▪&nbsp; 실행 전 필수 라이브러리 준비**")
    prep_col_btn3, prep_col_msg3 = st.columns([1.4, 3.6])

    with prep_col_btn3:
        prepare_clicked3 = st.button(
            "필수 라이브러리 설치/점검",
            icon=":material/build:",
            use_container_width=True,
            key="prepare_required_runtime_btn3",
        )

    with prep_col_msg3:
        if st.session_state.get("required_runtime_ready3"):
            st.success("✅ 필수 실행 환경 준비 완료")
        else:
            st.caption("최초 실행 또는 배포 직후에는 먼저 설치/점검을 완료한 뒤 기관검색을 진행하세요.")
    ...
    if prepare_clicked3:
        with st.spinner("Playwright Chromium 설치 및 실행 가능 여부를 확인 중입니다..."):
            ok3, msg3 = page1_org_metadata._prepare_required_runtime_before_org_search()

        st.session_state.required_runtime_ready3 = bool(ok3)
    ...
    if not st.session_state.get("required_runtime_ready3"):
        st.info("먼저 [필수 라이브러리 설치/점검] 버튼을 눌러 실행 환경을 준비해주세요.")
        st.stop()
    ...
    if info3 and info3.get("total_pages", 0) > 0:
        ...
        if run_download:
            ...
            expected_pages = max(1, math.ceil(int(info3.get("total_count") or 0) / 100))
            progress_bar, status_box, update_progress, clear_progress_status = make_progress_box("파일데이터 다운로드 준비 중...")

            download_state = {
                "current": 0,
                "total": int(info3.get("total_count") or 0),
                "current_saved": 0,
                "past_saved": 0,
                "failed": 0,
            ...
            def _elapsed_text():
                elapsed = max(0, int(time.time() - download_state["start_time"]))
            ...
            def update_status(msg):
                ...
                if page_match:
                    ...
                    display_msg = (
                        ...
                        f"({ratio * 100:.1f}%) / "
                        f"현재데이터 저장 {download_state['current_saved']:,}건 / "
                        f"과거데이터 저장 {download_state['past_saved']:,}건 / "
                        f"실패 {download_state['failed']:,}건 / "
                        f"경과 {_elapsed_text()}"
                    )

                elif "현재데이터 저장" in msg:
                    download_state["current_saved"] += 1
                    current = int(download_state["current"] or 0)
                    total = max(int(download_state["total"] or 1), 1)
                    ...
                    percent = 98
                    display_msg = f"전체 다운로드 완료. ZIP 파일 생성 중... / 경과 {_elapsed_text()}"

                else:
                    display_msg = f"{msg} / 경과 {_elapsed_text()}"

                update_progress(percent, display_msg)

            with st.spinner(f"'{org}' 파일데이터 다운로드 중..."):
                try:
                    zip_path = run_file_download_subprocess(
                        inst_name=org,
                        org_url=org_url,
                    ...
                    update_progress(100, "파일데이터 다운로드 및 ZIP 생성이 완료되었습니다.")
                    clear_progress_status()
                    if not zip_path or not os.path.exists(zip_path):
                        st.error("ZIP 파일 생성에 실패했습니다.")
                    else:
                        st.success("🌟 파일데이터 다운로드 및 ZIP 생성이 완료되었습니다.")
                        col_zip_btn, col_zip_msg = st.columns([1.2, 3.8], vertical_alignment="center")
                        with col_zip_btn:
                        ...
                        with col_zip_msg:
                            st.caption("ZIP 파일 용량과 기관별 파일데이터 수에 따라 다운로드 준비 및 저장에 다소 시간이 소요될 수 있습니다.")
                except Exception as e:
                    update_progress(100, "파일 다운로드 중 오류가 발생했습니다.")
                    clear_progress_status()
