    ...
    with tab1:
        ...
        if st.button("전체 수집 (백그라운드 실행)", type="primary", use_container_width=True):
            try:
                ...
                subprocess.Popen(
                    ["python", "crawler_metadata.py"], 
                    creationflags=subprocess.CREATE_NEW_CONSOLE
                )
                st.success("✅ 크롤러 엔진이 별도의 터미널 창에서 성공적으로 실행되었습니다! 작업 표시줄을 확인해 주세요.")
            except Exception as e:
...
# ==========================================
    ...
    # 분리한 파일(page3_download.py)의 render_page 함수 실행
    page3_download.render_page(get_soup, find_valid_org_name, BASE_URL, HEADERS)