...
class _ProgressStdout:
    """
    crawler_metadata.py가 print로 출력하는 실제 진행 로그를 Streamlit 진행바와 연결합니다.
    크롤링 로직은 변경하지 않고 stdout 로그만 실시간으로 해석합니다.
    """
    def __init__(self, buffer, line_callback):
        self.buffer = buffer
    ...
    def flush(self):
        if self._pending.strip():
            try:
                self.line_callback(self._pending)
            except Exception:
                pass
            self._pending = ""
        try:
...
def render_tab2(
    ...
    try:
        ...
        log_box = st.empty()
        status_box = st.empty()
        progress_detail_box = st.empty()
        log_buffer = io.StringIO()
        progress_started_at = time.perf_counter()

        def _elapsed_text():
            elapsed = int(time.perf_counter() - progress_started_at)
        ...
        def _set_progress(percent, message, detail=""):
            ...
            if detail:
                progress_detail_box.caption(detail)

        def update_status(msg):
            _set_progress(None, msg)

        def _handle_progress_line(line):
            line = _clean(line)
            if not line:
                return

            # 1) 목록 URL 수집 단계: crawler_metadata.py의 [LIST] 로그를 해석합니다.
            m = re.search(r"\[LIST\]\s*page\s*(\d+)\s*/\s*(\d+).*수집 중", line)
            if m:
                page_no = int(m.group(1))
                max_page_no = max(int(m.group(2)), 1)
                percent = 5 + int(min(page_no / max_page_no, 1) * 20)
                _set_progress(
                    percent,
                    f"상세 URL 목록 수집 중... {page_no}/{max_page_no}페이지",
                    f"경과시간 {_elapsed_text()} · 현재 {page_no}페이지 접근 중",
                )
                return
            ...
            if m:
                page_no = int(m.group(1))
                page_count = int(m.group(2).replace(',', ''))
                collected = int(m.group(3).replace(',', ''))
                max_items = max(int(m.group(4).replace(',', '')), 1)
                # max_items는 대부분 넉넉한 상한값이므로 URL 목록 단계는 25%까지만 표시합니다.
                percent = min(29, 8 + int(min(page_no / max(engine_max_pages or page_no or 1, 1), 1) * 21))
                _set_progress(
                    percent,
                    f"상세 URL 목록 수집 중... 누적 {collected:,}건",
                    f"경과시간 {_elapsed_text()} · {page_no}페이지에서 {page_count:,}건 발견",
                )
                return

            m = re.search(r"상세 URL 수집 완료.*?([\d,]+)건", line)
            if m:
                total = int(m.group(1).replace(',', ''))
                _set_progress(
                    30,
                    f"상세 URL {total:,}건 수집 완료. 상세 메타데이터 수집을 시작합니다...",
                    f"경과시간 {_elapsed_text()} · 상세 수집 대상 {total:,}건",
                )
                return

            # 2) 상세 HTML 수집/파싱 단계: 실제 완료 건수 기반으로 30~95%를 표시합니다.
            if "DETAIL" in line:
                m = re.search(
                    r"\[.*?DETAIL.*?\]\s*([\d,]+)\s*/\s*([\d,]+).*?성공\s*([\d,]+).*?실패\s*([\d,]+)",
                    line,
                )
                if m:
                    done = int(m.group(1).replace(',', ''))
                    total = max(int(m.group(2).replace(',', '')), 1)
                    ok = int(m.group(3).replace(',', ''))
                    fail = int(m.group(4).replace(',', ''))
                    ratio = min(done / total, 1)
                    percent = 30 + int(ratio * 65)
                    _set_progress(
                        percent,
                        f"상세 메타데이터 수집 중... {done:,}/{total:,}건 ({ratio * 100:.1f}%)",
                        f"성공 {ok:,}건 · 실패/보류 {fail:,}건 · 경과시간 {_elapsed_text()}",
                    )
                return

            # 3) 후순위/Playwright 회수 단계는 상세 수집 마무리 구간으로 표시합니다.
            if "후순위 처리" in line or "Playwright" in line:
                _set_progress(
                    95,
                    "일부 실패/보류 URL을 후순위로 정리 중입니다...",
                    f"경과시간 {_elapsed_text()} · {line[:160]}",
                )
                return

            if "기관별 메타데이터 수집 완료" in line or "기관별 수집 완료" in line:
                _set_progress(97, "수집 결과를 정리하는 중입니다...", f"경과시간 {_elapsed_text()}")

        progress_stdout = _ProgressStdout(log_buffer, _handle_progress_line)
        old_detail_progress_every = getattr(crawler_metadata, "DETAIL_PROGRESS_EVERY", None)

        try:
            # 진행바 정확도를 높이기 위해 상세 진행 로그 주기만 일시적으로 1건 단위로 낮춥니다.
            # 수집 URL/파싱/저장 로직 자체는 변경하지 않습니다.
            if old_detail_progress_every is not None:
                crawler_metadata.DETAIL_PROGRESS_EVERY = 1
            ...
            if old_detail_progress_every is not None:
                crawler_metadata.DETAIL_PROGRESS_EVERY = old_detail_progress_every
