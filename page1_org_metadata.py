...
def _prepare_required_runtime_before_org_search():
    """
    기관 검색 전에 Playwright Chromium 실행 가능 여부만 점검합니다.
    기존 crawler_metadata.py 내부의 Playwright 설치/실행 보정 로직은 그대로 사용합니다.
    기관별 URL 생성, 검색, 수집, 진행바, 엑셀 생성 로직은 변경하지 않습니다.
    """
    browser = None
    try:
        ...
        with sync_playwright() as p:
            if hasattr(crawler_metadata, "_launch_chromium_sync"):
                browser = crawler_metadata._launch_chromium_sync(p, headless=True)
            else:
        ...
        return True, "필수 실행 환경 준비가 완료되었습니다."
...
def render_tab2(
    ...
    st.markdown("**▪&nbsp; 실행 전 필수 라이브러리 설치/점검**")
