...
def render_tab2(get_soup, find_valid_org_name, format_tel_no, BASE_URL, HEADERS, ALL_SELECTABLE_COLUMNS, TARGET_METADATA_KEYS, METADATA_KEY_MAP):
    ...
    if st.session_state.get("total_pages1", 0) > 0:
        ...
        if "warning_msg1" not in st.session_state: st.session_state.warning_msg1 = ""
        if "selected_cols1" not in st.session_state: st.session_state.selected_cols1 = []
        if "prev_selected_cols1" not in st.session_state: st.session_state.prev_selected_cols1 = []
            ...
            if not selected_columns:
                ...
                try:
                    ...
                    if "모두 선택" in selected_columns:
                        target_columns = ALL_SELECTABLE_COLUMNS
                    else:
                    ...
                    # URL 수집
                    per_page = 100 
                    fast_total_pages = math.ceil((pages * 10) / per_page)
                    
                    my_bar.progress(5, text="URL 수집 중...")
                    
                    for page in range(1, fast_total_pages + 1):
                        list_url = f"{base_list_url}?dType=FILE&sort=updtDt&currentPage={page}&perPage={per_page}&org={encoded_org}"
                        try:
                    ...
                    # 🔍 2단계: 메타데이터 추출 및 엑셀 다운로드
                    if total_urls == 0:
                        st.error("수집할 URL이 없습니다. 기관명이나 사이트 상태를 확인해주세요.")
                        my_bar.empty()
                    else:
                        # 1. 비동기 파싱 함수 정의
                        ...
                        async def run_concurrent_scraping(urls):
                            ...
                            async with httpx.AsyncClient(headers=HEADERS, limits=limits, timeout=timeout, verify=False) as client:
                                async def worker():
                                    nonlocal completed
                        ...
                        # 3. 비동기 엔진 가동
                        my_bar.progress(10, text=f"준비 중... (총 {total_urls}건)")
                        ...
                        my_bar.empty()
                        st.success(f"수집 완료! 총 {len(rows)}건의 데이터를 성공적으로 추출했습니다.")
