...
def render_tab2(
    ...
    try:
        ...
        # - URL 수집/상세 HTML 수집/파싱 로직은 건드리지 않음
        # - 빈 값은 보정하지 않음
        # - 목록 카드/컬럼목록/설명 문구가 제공기관으로 잘못 들어간 경우만 기관명으로 교정
        # --------------------------------------------------
        if not result_df.empty and "제공기관" in result_df.columns:
            ...
            def _is_polluted_provider_value(value):
                ...
                # 빈 값은 상세 HTML 수집/파싱 실패 가능성이 있으므로 여기서 임의 보정하지 않음
                if not v:
                    return False
                ...
                # 컬럼목록/설명/데이터명 일부가 제공기관으로 잘못 들어간 대표 패턴
                pollution_signals = [
                    ",",
                    "제공항목",
                    "항목명",
