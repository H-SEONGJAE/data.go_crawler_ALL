...
def _run_metadata_crawler_for_url_compat(**kwargs):
    """
    crawler_metadata.py 버전 차이를 흡수해서 기관별 메타데이터 수집 함수를 호출합니다.
    - 최신 함수가 retry_wait_sec를 지원하면 0초로 넘겨 3분 대기 없이 즉시 재수집합니다.
    - 구버전 함수에 없는 인자는 자동 제외해 TypeError를 방지합니다.
    """
    fn = crawler_metadata.run_metadata_crawler_for_url
    try:
...
def render_tab2(
    ...
    try:
        ...
        # 기관별 필터 기능만 유지하고, 실제 수집 하이퍼파라미터는
        # crawler_metadata.py 전체 메타데이터 수집 설정값을 그대로 사용합니다.
        list_per_page = int(getattr(crawler_metadata, "LIST_PER_PAGE", 1000))
        engine_max_pages = int(getattr(crawler_metadata, "MAX_PAGES", 0))
        engine_max_detail_items = int(getattr(crawler_metadata, "MAX_DETAIL_ITEMS", 0))
        engine_detail_concurrency = int(getattr(crawler_metadata, "DETAIL_CONCURRENCY", 20))
        ...
        my_bar.progress(5, text="crawler_metadata.py 기준 기관별 수집 엔진 준비 중... 실패 URL은 대기 없이 즉시 재수집합니다.")
        ...
        def update_progress(payload):
            ...
            message = payload.get("message", "진행 중...") if isinstance(payload, dict) else "진행 중..."
            ...
            if percent is None:
                if stage in ["list", "list_start"]:
                    ...
                    max_pages_for_progress = int(payload.get("max_pages") or engine_max_pages or 1) if isinstance(payload, dict) else max(engine_max_pages, 1)
                    percent = 5 + min(25, int((page / max(max_pages_for_progress, 1)) * 25))
                elif stage in ["list_done", "detail_start"]:
                    percent = 30
                elif stage == "detail":
                    done = int(payload.get("done") or 0) if isinstance(payload, dict) else 0
                    total = max(int(payload.get("total") or 1), 1) if isinstance(payload, dict) else 1
        ...
        with contextlib.redirect_stdout(log_buffer):
            result = _run_metadata_crawler_for_url_compat(
