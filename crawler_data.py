...
def ensure_playwright_browser_installed(status_callback=None):
    """
    Streamlit Cloud/GitHub 배포 환경에서는 pip install playwright만으로는
    실제 Chromium 브라우저 바이너리가 설치되지 않는 경우가 있습니다.
    이 함수는 launch 실패 전에/실패 후에 playwright install chromium을 1회 수행합니다.
    """
    try:
        update_status(status_callback, "🔧 Playwright Chromium 설치 상태 확인 중...")
        subprocess.run(
        ...
        update_status(status_callback, "✅ Playwright Chromium 설치 확인 완료")
...
def launch_chromium_with_auto_install(playwright, headless=True, status_callback=None):
    """브라우저 실행 실패 시 Chromium을 설치하고 1회 재시도합니다."""
    try:
        ...
        if not need_install:
            raise
        update_status(status_callback, "⚠️ Chromium 실행 파일이 없어 설치 후 재시도합니다.")
...
def collect_portal_files(
    ...
    with sync_playwright() as p:
        browser = launch_chromium_with_auto_install(p, headless=headless, status_callback=status_callback)
